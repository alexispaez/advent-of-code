...
procedure Day_2_2 is
   ...
   function Is_Dampener_Safe (Report : Natural_Vectors.Vector)
                              return Boolean is
   begin
      --  Try removing one level
      for I in Report.First_Index .. Report.Last_Index loop
         declare
            R : Natural_Vectors.Vector := Report;
         begin
            R.Delete (I);
...
begin
   ...
   while not End_Of_File (F) loop
      ...
      begin
         ...
         if Is_Safe (Report) or else Is_Dampener_Safe (Report) then
            Safe_Reports := @ + 1;
         end if;
      end;
   end loop;
